...
import styles from './OrdersCard.module.less'
import {TOrderInfo} from "../../utils/types.ts";
import {useNavigate} from "react-router-dom";
import {PAGE_URI} from "../../utils/const.ts";

const OrdersCard = ({number, name, date, ingredients, status}: TOrderInfo) => {
    ...
    const openFeedDetail = () => {
        navigate(PAGE_URI.orders+'/'+number, {state: {showInModal: true}});
    }
    ...
    return (
        <div className={styles.feed__card} onClick={openFeedDetail} key={number}>
            ...
            <div className="text text_type_main-default mt-6">{status}</div>
